...
/* Helper to extract NVX-KeyServer version information.  Aborts on any failure. */
...
abort:
    freeReplyObject(reply);
    fprintf(stderr, "Error:  Cannot determine NVX-KeyServer version, aborting\n");
    exit(1);
}
...
static int detect_debug_sleep(redisContext *c) {
    ...
    if (reply == NULL || c->err) {
        ...
        fprintf(stderr, "Error testing for DEBUG SLEEP (NVX-KeyServer error: %s), exiting\n", cause);
        exit(-1);
    }
