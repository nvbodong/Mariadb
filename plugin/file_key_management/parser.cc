...
bool Parser::read_filekey(const char *filekey, char *secret)
{
  ...
  {
    std::string base64_encrypt_str(secret, len);
    assert(len >0);
    std::string enc_decoded = base64_decode(base64_encrypt_str);
    std::string r;
    r = kv_cli.decrypt(enc_decoded);
    std::memset(secret,0, MAX_SECRET_SIZE);

    assert(r.length() < MAX_SECRET_SIZE);
    std::memcpy(secret,r.c_str(), r.length() );
    len = r.length();
  }
...
{
  std::string base64_encrypt_str,r,enc_decoded;
  int f;
  ...
  {
    base64_encrypt_str.assign((const char*)buffer, file_size);
    enc_decoded = base64_decode(base64_encrypt_str);
    r = kv_cli.decrypt(enc_decoded);
    std::memset(buffer,0, file_size);

    assert(r.length() < file_size);
    std::memcpy(buffer,r.c_str(), r.length() );
    file_size = r.length();
  }
