...
namespace paddle {
    namespace framework {
        ...
        void AESCipher::Init(const std::string &cipher_name, const int &iv_size, const int &tag_size) {
        ...
        std::string AESCipher::EncryptInternal(const std::string &plaintext, const std::string &key) {
            CryptoPP::member_ptr <CryptoPP::SymmetricCipher> m_cipher;
            CryptoPP::member_ptr <CryptoPP::StreamTransformationFilter> m_filter;
            bool need_iv = false;
            const unsigned char *key_char = reinterpret_cast<const unsigned char *>(&(key.at(0)));
            BuildCipher(true, &need_iv, &m_cipher, &m_filter);
            if (need_iv) {
                iv_ = CipherUtils::GenKey(iv_size_);
                m_cipher.get()->SetKeyWithIV(key_char, key.size(),
        ...
        std::string AESCipher::DecryptInternal(const std::string &ciphertext, const std::string &key) {
            CryptoPP::member_ptr <CryptoPP::SymmetricCipher> m_cipher;
            CryptoPP::member_ptr <CryptoPP::StreamTransformationFilter> m_filter;
            bool need_iv = false;
            const unsigned char *key_char = reinterpret_cast<const unsigned char *>(&(key.at(0)));
        ...
        std::string AESCipher::AuthenticatedEncryptInternal(const std::string &plaintext,
        ...
        std::string AESCipher::AuthenticatedDecryptInternal(const std::string &ciphertext,
                                                            const std::string &key) {
            CryptoPP::member_ptr <CryptoPP::AuthenticatedSymmetricCipher> m_cipher;
            ...
            CryptoPP::StringSource(ciphertext.substr(ciphertext_beg), true,
                                   new CryptoPP::Redirector(*m_filter));
            PADDLE_ENFORCE_EQ(m_filter->GetLastResult(), true,
        ...
        void
        AESCipher::BuildCipher(bool for_encrypt, bool *need_iv,
                               CryptoPP::member_ptr <CryptoPP::SymmetricCipher> *m_cipher,
                               CryptoPP::member_ptr <CryptoPP::StreamTransformationFilter> *m_filter) {
            if (aes_cipher_name_ == "AES_ECB_PKCSPadding" && for_encrypt) {
                ...
                m_filter->reset(new CryptoPP::StreamTransformationFilter(
                        *(*m_cipher).get(), NULL, CryptoPP::BlockPaddingSchemeDef::PKCS_PADDING));
        ...
        void AESCipher::BuildAuthEncCipher(
                bool *need_iv, CryptoPP::member_ptr <CryptoPP::AuthenticatedSymmetricCipher> *m_cipher,
                CryptoPP::member_ptr <CryptoPP::AuthenticatedEncryptionFilter> *m_filter) {
            if (aes_cipher_name_ == "AES_GCM_NoPadding") {
                ...
                *need_iv = true;
                m_filter->reset(new CryptoPP::AuthenticatedEncryptionFilter(
                        *(*m_cipher).get(), NULL, false, tag_size_ / 8, CryptoPP::DEFAULT_CHANNEL,
            ...
            if (aes_cipher_name_ == "AES_GCM_NoPadding") {
                ...
                *need_iv = true;
                m_filter->reset(new CryptoPP::AuthenticatedDecryptionFilter(
                        *(*m_cipher).get(), NULL, CryptoPP::AuthenticatedDecryptionFilter::DEFAULT_FLAGS,
                        tag_size_ / 8, CryptoPP::BlockPaddingSchemeDef::NO_PADDING));
            } else {
                PADDLE_THROW(paddle::platform::errors::Unimplemented(
                        "Create cipher error. "
                        "Cipher name %s is error, or has not been implemented.",
        ...
        std::string AESCipher::Encrypt(const std::string &plaintext, const std::string &key) {
        ...
        std::string AESCipher::Decrypt(const std::string &ciphertext, const std::string &key) {
            return is_authenticated_cipher_ ? AuthenticatedDecryptInternal(ciphertext, key)
                                            : DecryptInternal(ciphertext, key);
        }

        void AESCipher::EncryptToFile(const std::string &plaintext, const std::string &key,
        ...
        std::string AESCipher::DecryptFromFile(const std::string &key, const std::string &filename) {
