...
namespace paddle {
    namespace framework {
        ...
        std::vector<std::string> name_list({"AES_CTR_NoPadding", "AES_CBC_PKCSPadding",
                                            "AES_ECB_PKCSPadding", "AES_GCM_NoPadding"});
...
auto &i
: input) {
std::string ciphertext = cipher->Encrypt(std::to_string(i), AESTest::key);
...
auto cipher = CipherFactory::CreateCipher("aes_test.conf");
cipher->
EncryptToFile(plaintext, AESTest::key, filename
);
