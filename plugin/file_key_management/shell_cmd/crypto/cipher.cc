...
namespace paddle {
    namespace framework {
        ...
        std::shared_ptr <Cipher> CipherFactory::CreateCipher(const std::string &config_file) {
            ...
            if (cipher_name.find("AES") != cipher_name.npos) {
                ...
                if (config_file.empty() || !CipherUtils::GetValue<int>(config, "iv_size", &iv_size)) {
                    iv_size = CipherUtils::AES_DEFAULT_IV_SIZE;
                }
                // if not set tag_size, set default value
                if (config_file.empty() || !CipherUtils::GetValue<int>(config, "tag_size", &tag_size)) {
