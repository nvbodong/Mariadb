...
namespace paddle {
    namespace framework {
        ...
        std::string CipherUtils::GenKey(int length) {
            ...
            prng.GenerateBlock(reinterpret_cast<unsigned char *>(&(rng.at(0))), rng.size());
            return rng;
        }
        ...
        std::string CipherUtils::GenKeyToFile(int length, const std::string &filename) {
            ...
            PADDLE_ENFORCE_EQ(
                    fout.is_open(), true,
                    paddle::platform::errors::Unavailable("Failed to open file : %s, "
        ...
        std::string CipherUtils::ReadKeyFromFile(const std::string &filename) {
            ...
            std::string ret{std::istreambuf_iterator<char>(fin), std::istreambuf_iterator<char>()};
            fin.close();
            return ret;
        }

        std::unordered_map<std::string, std::string>
        CipherUtils::LoadConfig(const std::string &config_file) {
        ...
        template<>
        bool CipherUtils::GetValue<bool>(const std::unordered_map<std::string, std::string> &config,
