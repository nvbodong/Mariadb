...
namespace paddle {
    namespace framework {
        ...
        static std::shared_ptr<FILE> fs_open_internal(const std::string &path, bool is_pipe,
        ...
        size_t localfs_buffer_size() {
            return localfs_buffer_size_internal();
        }

        void localfs_set_buffer_size(size_t x) {
            localfs_buffer_size_internal() = x;
        }

        std::shared_ptr<FILE> localfs_open_read(std::string path, const std::string &converter) {
            bool is_pipe = false;
        ...
        std::shared_ptr<FILE> localfs_open_write(std::string path, const std::string &converter) {
        ...
        int64_t localfs_file_size(const std::string &path) {
            ...
            if (0 != stat(path.c_str(), &buf)) {
                std::cout << (("Failed to get file status via stat function."));
                return -1;
            }
        ...
        std::vector<std::string> localfs_list(const std::string &path) {
            ...
            int err_no = 0;
            pipe = shell_popen(string::format_string("find %s -type f -maxdepth 1", path.c_str()), "r",
                               &err_no);
        ...
        bool localfs_exists(const std::string &path) {
            std::string test_f =
                    shell_get_command_output(string::format_string("[ -f %s ] ; echo $?", path.c_str()));
            ...
            std::string test_d =
                    shell_get_command_output(string::format_string("[ -d %s ] ; echo $?", path.c_str()));
        ...
        size_t hdfs_buffer_size() {
            return hdfs_buffer_size_internal();
        }

        void hdfs_set_buffer_size(size_t x) {
            hdfs_buffer_size_internal() = x;
        }
        ...
        const std::string &hdfs_command() {
            return hdfs_command_internal();
        }

        void hdfs_set_command(const std::string &x) {
            hdfs_command_internal() = x;
        }
        ...
        const std::string &download_cmd() {
            return customized_download_cmd_internal();
        }
        ...
        std::shared_ptr<FILE> hdfs_open_read(std::string path, int *err_no,
            ...
            if (download_cmd() != "") {    // use customized download command
                path = string::format_string("%s \"%s\"", download_cmd().c_str(), path.c_str());
            } else {
                if (fs_end_with_internal(path, ".gz")) {
                    path =
                            string::format_string("%s -text \"%s\"", hdfs_command().c_str(), path.c_str());
                } else {
                    path =
                            string::format_string("%s -cat \"%s\"", hdfs_command().c_str(), path.c_str());
                }
            }
        ...
        std::shared_ptr<FILE> hdfs_open_write(std::string path, int *err_no,
                                              const std::string &converter) {
            path = string::format_string("%s -put - \"%s\"", hdfs_command().c_str(), path.c_str());
            bool is_pipe = true;
        ...
        void hdfs_remove(const std::string &path) {
            ...
            shell_execute(string::format_string("%s -rmr %s &>/dev/null; true", hdfs_command().c_str(),
                                                path.c_str()));
        }
        ...
        std::vector<std::string> hdfs_list(const std::string &path) {
            ...
            do {
                err_no = 0;
                std::shared_ptr<FILE> pipe;
                pipe = shell_popen(string::format_string("%s -ls %s | ( grep ^- ; [ $? != 2 ] )",
        ...
        std::string hdfs_tail(const std::string &path) {
            ...
            return shell_get_command_output(
                    string::format_string("%s -text %s | tail -1 ", hdfs_command().c_str(), path.c_str()));
        }
        ...
        void hdfs_mkdir(const std::string &path) {
            ...
            shell_execute(
                    string::format_string("%s -mkdir %s; true", hdfs_command().c_str(), path.c_str()));
        }
        ...
        void hdfs_mv(const std::string &src, const std::string &dest) {
            ...
            shell_execute(string::format_string("%s -mv %s %s; true", hdfs_command().c_str(),
        ...
        std::shared_ptr<FILE> fs_open_read(const std::string &path, int *err_no,
            ...
            switch (fs_select_internal(path)) {
                ...
                default:
                    std::cout << (("Unsupport file system. Now only supports local file system and "
                                   "HDFS."));
            }
        ...
        std::shared_ptr<FILE> fs_open(const std::string &path, const std::string &mode, int *err_no,
                                      const std::string &converter) {
            if (mode == "r" || mode == "rb") {
            ...
            std::cout << "Unsupport file open mode: " << mode
                      << ". Only supports 'r', 'rb', 'w' or 'wb'.";
            return {};
        }
        ...
        int64_t fs_file_size(const std::string &path) {
            switch (fs_select_internal(path)) {
                ...
                default:
                    std::cout << ("Unsupport file system. Now only supports local file system.");
            }
