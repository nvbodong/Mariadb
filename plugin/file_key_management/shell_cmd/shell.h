...
#if defined(__arm__) || defined(__aarch64__) || defined(__ARM_NEON) || defined(__ARM_NEON__)
#define PADDLE_ARM
#endif
...
namespace paddle {
    namespace framework {
        ...
        std::shared_ptr<FILE> shell_popen(const std::string &cmd, const std::string &mode, int *err_no,
                                          int *status = NULL, bool redirect_stderr = false);

        extern std::pair<std::shared_ptr<FILE>, std::shared_ptr<FILE>>
        shell_p2open(const std::string &cmd);
        ...
        // sleep_inter:ms, default -1 means not sleep.
        extern std::vector<std::string> shell_execute_cmd(const std::string &cmd, int time_out = 0,
                                                          int sleep_inter = 0,
